...
def kmeans_evaluate(X, Y, n_clusters=50, n_init=20, random_state=0, epoch=0, model_save_dir="", name=""):
    ...
    np.save(os.path.join(model_save_dir, f"{epoch:04d}_{name}_y_pred_new.npy"), cluster_ids)
    return {
        "kmeans_acc": acc,
        "silhouette": sil,
...
def run_once(hyper_dir, model_type="large", pretype="f", posttype="f", sel="full", preepochs=20, postepochs=20, on_site_eval=False, save_eval=True, eval_train=False, save_model=False): 
    ...
    valid_accs = ListRecorder(os.path.join(model_save_dir, "valid.newacc"))
    full_valid_accs = ListRecorder(os.path.join(model_save_dir, "full_valid.newacc"))

    valid_sils = ListRecorder(os.path.join(model_save_dir, "valid.newsil"))
    full_valid_sils = ListRecorder(os.path.join(model_save_dir, "full_valid.newsil"))

    valid_aris = ListRecorder(os.path.join(model_save_dir, "valid.newari"))
    full_valid_aris = ListRecorder(os.path.join(model_save_dir, "full_valid.newari"))

    valid_dbis = ListRecorder(os.path.join(model_save_dir, "valid.newdbi"))
    full_valid_dbis = ListRecorder(os.path.join(model_save_dir, "full_valid.newdbi"))
    ...
    for epoch in range(BASE, BASE + preepochs):
        ...
        valid_res = kmeans_evaluate(valid_z, valid_y, n_clusters=38, 
                                    epoch=epoch, model_save_dir=model_save_dir, name="valid")
        full_valid_res = kmeans_evaluate(full_valid_z, full_valid_y, n_clusters=38, 
