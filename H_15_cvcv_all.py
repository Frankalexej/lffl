...
def run_once(hyper_dir, model_type="large", pretype="f", posttype="f", sel="full"): 
    ...
    for epoch in range(BASE, BASE + EPOCHS):
        ...
        # trainlike eval
        model.eval()
        trainlike_valid_loss = 0.
        trainlike_valid_num = len(trainlikevalid_loader_1)
        trainlike_valid_correct = 0
        trainlike_valid_total = 0
        for idx, (x, y) in enumerate(trainlikevalid_loader_1):
            ...
            y_hat = model(x)
            loss = criterion(y_hat, y)
            trainlike_valid_loss += loss.item()

            pred = model.predict_on_output(y_hat)

            trainlike_valid_total += y_hat.size(0)
            trainlike_valid_correct += (pred == y).sum().item()

        trainlikevalid_losses.append(trainlike_valid_loss / trainlike_valid_num)
        trainlikevalid_accs.append(trainlike_valid_correct / trainlike_valid_total)

        train_losses.save()
        valid_losses.save()
        full_valid_losses.save()
        trainlikevalid_losses.save()
        train_accs.save()
        valid_accs.save()
        ...
        if epoch % 5 == 0:
            draw_learning_curve_and_accuracy(losses=(train_losses.get(), valid_losses.get(), full_valid_losses.get(), trainlikevalid_losses.get()), 
    ...
    for epoch in range(BASE, BASE + EPOCHS):
        ...
        model.eval()
        trainlike_valid_loss = 0.
        trainlike_valid_num = len(trainlikevalid_loader_2)
        trainlike_valid_correct = 0
        trainlike_valid_total = 0
